...
export default function TextForm(props) {
    ...
    useEffect(() => {
        if (readAloud) {
            ...
            speechInstance.onend = () => setReadAloud(false); // Reset readAloud state when speech ends
    ...
    return (
        ...
        <div className="container" style={{color: props.mode==='dark'?'white':'#042743'}}> 
            <h1>{props.heading}</h1>
            <div className="mb-3"> 
            <textarea className="form-control" value={text} onChange={handleOnChange} style={{backgroundColor: props.mode==='dark'?'rgb(62 90 176)':'white', color: props.mode==='dark'?'white':'#042743'}} id="myBox" rows="8"></textarea>
            </div>
            ...
            <button disabled={text.length === 0} className="btn btn-primary mx-1" onClick={() => setReadAloud(true)}>
                    Read Text Aloud
            </button>
            <button className="btn btn-danger mx-1" onClick={handleStopClick}>
                    Stop Reading
            </button>
        </div>
        <div className="container my-3" style={{color: props.mode==='dark'?'white':'#042743'}}>
            <h2>Your text summary</h2>
            <p>{text.trim().length > 0 ? text.split(/\s+/).filter(word => word.trim() !== '').length : 0} words and {text.length} characters</p>
